...
class Notification(models.Model):
    noti_receiver = models.ForeignKey(
        User, on_delete=models.CASCADE, default="", related_name="noti_receiver"
    )
    type_of_noti = models.CharField(max_length=30, choices=TYPE_OF_NOTI, default="")
    url = models.URLField(null=True, blank=True)
    date_created = models.DateTimeField(auto_now_add=True)
    is_read = models.BooleanField(default=False)
    question_noti = models.ForeignKey(
        Question, on_delete=models.CASCADE, blank=True, null=True
    )
    answer_noti = models.ForeignKey(
...
class PrivRepNotification(models.Model):
    ...
    is_read = models.BooleanField(default=False)
    type_of_PrivNotify = models.CharField(
        max_length=30, choices=PRIV_NOTIFY_CHOICES, default=""
    )
    ...
    question_priv_noti = models.ForeignKey(
        Question, on_delete=models.CASCADE, blank=True, null=True
    )
    answer_priv_noti = models.ForeignKey(
        Answer, on_delete=models.CASCADE, blank=True, null=True
    )
