...
from qa.models import Answer, ProtectQuestion, Question
...
def answer_with_extreme_votes_Mod_Tools(request):
    answers = Answer.objects.annotate(countVotes=Count("a_vote_ups")).order_by(
        "-countVotes"
    )
...
def questions_withmost_commts_vws_edts_Mod_Tools(request):
    ...
    context = {
        "questions_comnts": questions_comnts,
        "question_vws": question_vws,
        "question_edts": question_edts,
    }
...
def recentlyClsd_Mod_Tools(request):
    closed_questions = ReviewCloseVotes.objects.filter(
        is_completed=True, reviewActions="Close"
    ).order_by("-date")
...
def recently_reopnd_Mod_Tools(request):
    reopened_questions = ReviewQuestionReOpenVotes.objects.filter(
        is_completed=True, reviewActions="OPEN"
    ).order_by("-date")
...
def pending_Close_Vts_Mod_Tools(request):
    pnding_close_votes = ReviewCloseVotes.objects.filter(is_completed=False).exclude(
        question_to_closed__is_closed=True
    )

    context = {
        "pnding_close_votes": pnding_close_votes,
    }
    return render(request, "tools/pending_Close_Vts_Mod_Tools.html", context)


def pending_reopen_Vts_Mod_Tools(request):
    pnding_reopen_votes = ReviewQuestionReOpenVotes.objects.filter(is_completed=False)

    context = {
        "pnding_reopen_votes": pnding_reopen_votes,
    }
...
def newAns_To_oldQns_Mod_Tools(request):
    ...
    new_answers = (
        ...
        .filter(questionans__date__lte=timezone.now() - timedelta(days=2))
        .filter(date__gte=timezone.now() - timedelta(days=10))
    )
