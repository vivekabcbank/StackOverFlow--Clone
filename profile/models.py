...
from qa.models import CommentQ, Question


def random_img():
    dir_path = "media/"
    files = [
        content for content in listdir(dir_path) if isfile(path_join(dir_path, content))
    ]
...
class Profile(models.Model):
    ...
    profile_photo = models.ImageField(
        upload_to="profile_photos", default="media/isle.jpg"
    )
    about_me = models.CharField(max_length=30, default="", blank=True, null=True)
    website_link = models.URLField(blank=True)
    twitter_link = models.URLField(blank=True)
    github_link = models.URLField(blank=True)
    bookmark_questions = models.ManyToManyField(
        Question, related_name="bookmark_questions", blank=True
    )
    ...
    min_expierence_level = models.CharField(
        max_length=30, choices=EXPERIENCE_LEVEL, default=""
    )
    max_expierence_level = models.CharField(
        max_length=30, choices=EXPERIENCE_LEVEL, default=""
    )
    ...
    Refiner_Illuminator_TagPostCounter = models.IntegerField(
    ...
    def get_absolute_url(self):
        return reverse(
            "profile:activityPageTabProfile",
            kwargs={"user_id": self.user_id, "username": self.user.username},
        )
