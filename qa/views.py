...
from .decorators import highModRequired, loggedOutFromAllDevices, superuser_only
from .forms import (
    AnswerForm,
    BanUser_Form,
    BountyForm,
...
def InlineTagEditingForm(request, question_id):
    ...
    if is_ajax(request) and request.method == "POST":
        form = InlineTagEditForm(instance=data, data=request.POST, files=request.FILES)
        if form.is_valid():
...
def save_comment(request, question_id):
    ...
    if request.method == "POST":
        if request.user.profile.comment_everywhere_Priv:
            ...
            getComments = CommentQ.objects.filter(commented_by=request.user).count()
            if getComments >= 10:
            ...
            if comment != "":
                ...
                if "@" in getCommentBody:
                    try:
                        ...
                        getUserIdByUsername = User.objects.get(username=newWord)
            ...
            return JsonResponse({"action": "Need atleast 50 Reputation to Comment"})
...
def answerTimeline(request, answer_id):
    ...
    results = sorted(chain(get_comments, edit_history), key=ordering, reverse=True)
...
def end_bounty_thread(id):
    ...
    print(
        "Task Successfully Completed " + " Bounty of a Question is Successfully Removed"
    )
...
def AjaxFlagForm(request, question_id):
    ...
    getCreateFlag_object = (
        FlagPost.objects.filter(question_forFlag=data).exclude(ended=True).first()
    )
        ...
        if Flag_Form.is_valid():
            ...
            if request.user.profile.flag_posts:
                if formData == "SPAM" or formData == "RUDE_OR_ABUSIVE":
                    getCreateFlag_object = (
                        FlagPost.objects.filter(question_forFlag=data)
                        .filter(
                    ...
                    if getCreateFlag_object:
                        ...
                        (
                            create_Low_Quality_Post_Instance,
                            cre,
                        ) = LowQualityPostsCheck.objects.get_or_create(
                            suggested_by=request.user,
                            low_is=data,
                            why_low_quality="Very Low Quality",
                            suggested_through="User",
                        )
                        createLowQualityReviewInstance = (
                            ReviewLowQualityPosts.objects.get_or_create(
                                review_of=create_Low_Quality_Post_Instance,
                                is_question=data,
                            )
                        )
                        # return redirect('qa:questionDetailView', pk=data.id)
                    ...
                    formData == "IN_NEED_OF_MODERATOR_INTERVATION"
                    or formData == "ABOUT_PROFESSIONAL"
                ):
                    getCreateFlag_object = (
                        FlagPost.objects.filter(question_forFlag=data)
                        .filter(
                    ...
                    if getCreateFlag_object:
                        messages.error(request, "Previous Flag is Waiting for Review")
                    else:
                        new_post.flagged_by = request.user
                        new_post.question_forFlag = data
                        new_post.save()
                        ...
                        (
                            ...
                            created,
                        ) = ReviewFlagPost.objects.get_or_create(
                ...
                # "SEEKING_RECCOMENDATIONS" or:
                    ...
                    if getCreateFlag_object:
                        ...
                        (
                            createLowInstance,
                            cre,
                        ) = CloseQuestionVotes.objects.get_or_create(
                            user=request.user,
                            question_to_closing=data,
                            why_closing="Duplicate",
                            ended=False,
                        )
                        ...
                        (
                            createInstance,
                            created,
                        ) = ReviewCloseVotes.objects.get_or_create(
    ...
    if is_ajax(request) and request.method == "POST":
        ...
        if Flag_Form.is_valid():
            ...
            if request.user.profile.flag_posts:
                if formData == "SPAM" or formData == "RUDE_OR_ABUSIVE":
                    getCreateFlag_object = (
                        FlagPost.objects.filter(answer_forFlag=getAnswer)
                        .filter(
                            Q(actions_Flag_Q="SPAM")
                            | Q(actions_Flag_Q="RUDE_OR_ABUSIVE")
                        )
                    ...
                    getCreateFlag_object = (
                        FlagPost.objects.filter(
                            answer_forFlag=getAnswer, actions_Flag_Q="NOT_AN_ANSWER"
                        )
                        .exclude(ended=True)
                        .first()
                    )
    ...
    if is_ajax(request) and request.method == "POST":
        ...
        if bounty_form.is_valid():
            if request.user.profile.set_bounties:
                ...
                new_post.save()
                data.save()
                t = threading.Thread(target=end_bounty_thread, args=[new_post.id])
                t.setDaemon(True)
                t.start()
...
def ReOpenVotesAjax(request, question_id):
    ...
    get_LIVE_Reviwing_object = (
        ReviewQuestionReOpenVotes.objects.filter(question_opened=data)
                        ...
                        if s.tag_name in data.tags.all().values_list("name", flat=True):
                            ...
                            if formData == getCreatedObject.why_opening:
                                ...
                                (
                                    createInstance,
                                    created,
                                ) = ReviewQuestionReOpenVotes.objects.get_or_create(
...
def questionDetailView(
    ...
    for anss in answers_of_questions:
        ...
        getLastEditVotes = QuestionEditVotes.objects.filter(edited_answer=anss).last()
        if getLastEditVotes:
            if (
    ...
    countingActiveBounties = Question.objects.filter(
        limit_exced=True, is_bountied=True
    ).count()
        ...
        if form.is_valid():
            ...
            if data.is_protected:
                if request.user.profile.remove_new_user_restrictions:
                    ...
                    data.save()
                    question_URL = request.build_absolute_uri(data.get_absolute_url())
                    ...
                    if len(gettingBody) <= 200:
                        ...
                        (
                            ...
                            suggested_through="Automatic",
                            low_ans_is=new_post,
                            why_low_quality="Answer_Less_Than_200",
                            is_completed=False,
                        )
                        ReviewLowQualityPosts.objects.get_or_create(
                            review_of=create_Low_Quality_Post_Instance,
                ...
                if gettingWiki and request.user.profile.create_wiki_posts == False:
                    ...
                    messages.error(
                        request,
                        "You need atleast 10 Reputation to this Answer into Wiki Posts",
                    )
                else:
                ...
                getRecentAnswer = Answer.objects.filter(
                    answer_owner=request.user
                ).last()
                if getRecentAnswer and request.user.profile.editPostTimeOfUser:
                    if getEditingTime >= timezone.now() - timedelta(
                        minutes=5
                    ) and getRecentAnswer.date >= timezone.now() - timedelta(minutes=5):
                        ...
                        if (
                            request.user.profile.Refiner_Illuminator_TagPostCounter
                            >= 50
                        ):
                            TagBadge.objects.get_or_create(
                                awarded_to_user=request.user,
    ...
    # Question Views
    queryset = Question.objects.annotate(num_views=Count("viewers")).order_by(
        "-num_views"
    )
    ...
    if getClose_votes_on_this_question:
        if (
            datas.viewers.all().count() >= 20
            and getClose_votes_on_this_question.date
            <= timezone.now() - timedelta(days=5)
        ):
            print("Delete the Last Close Vote")
            CloseQuestionVotes.objects.filter(question_to_closing=data).last().delete()
    ...
    get_Ended_With_2 = ReviewCloseVotes.objects.filter(question_to_closed=data).last()
    if get_Ended_With_2:
    ...
    if request.method == "POST":
        ...
        if answerFlagForm.is_valid():
            ...
            if request.user.profile.flag_posts:
                if formData == "SPAM" or formData == "RUDE_OR_ABUSIVE":
                    getCreateFlag_object = (
                        FlagPost.objects.filter(answer_forFlag=data)
                        .filter(
                            Q(actions_Flag_Q="SPAM")
                            | Q(actions_Flag_Q="RUDE_OR_ABUSIVE")
                        )
                    ...
                    getCreateFlag_object = (
                        FlagPost.objects.filter(
                            answer_forFlag=data, actions_Flag_Q="NOT_AN_ANSWER"
                        )
                        .exclude(ended=True)
                        .first()
                    )
    ...
    getLastEditVotes = QuestionEditVotes.objects.filter(edited_question=data).last()
    ...
    if request.user.is_authenticated:
        is_flagged = FlagPost.objects.filter(
            question_forFlag=data, flagged_by=request.user
        ).first()
    else:
        is_flagged = ""
...
def flagComment(request, commentq_id):
    ...
    createTrackRecord = (
        FlagComment.objects.filter(comment_of=commentID).exclude(ended=True).first()
    )
        ...
        if FlagCommentForm.is_valid():
            ...
            if formData_unkind == "UNKIND" or formData_unkind == "NOT_NEEDED":
                ...
                if createTrackRecord:
                    ...
                    createTrackRecord.save()
                    (
                        createReviewingFlagCommentInstance,
                        created,
                    ) = ReviewFlagComment.objects.get_or_create(flag_of=commentID)
...
def save_comment_answer(request, answer_id):
    if request.method == "POST":
        if request.user.profile.comment_everywhere_Priv:
            ...
            if comment != "":
                create = CommentQ.objects.create(
                    answer_comment=ans, comment=comment, commented_by=request.user
                )
            ...
            if last_posted_q is None:
                ...
                for typedTags in formTags:
                    ...
                    if request.user.profile.create_tags:
                        if len(gettingBody) >= 0 and len(gettingBody) <= 29:
                            messages.error(
                                request,
                                "Body Text should atleast 30 words. You entered "
                                + str(len(gettingBody)),
                            )
                            ...
                            if len(gettingBody) <= 200:
                                (
                                    ...
                                    suggested_through="Automatic",
                                    low_is=new_post,
                                    why_low_quality="Question_Less_Than_200",
                                    is_completed=False,
                                )
                                createReview_item = (
                                    ReviewLowQualityPosts.objects.get_or_create(
                                        review_of=create_Low_Quality_Post_Instance,
                                        is_question=new_post,
                                        is_reviewed=False,
                                    )
                                )
                        ...
                        messages.error(
                            request,
                            f"You need atleast 1500 Reputation to create a New Tag - {formTags}",
                        )
                ...
                if not request.user.profile.remove_new_user_restrictions:
                    if last_posted_q.date >= is_olderThan_nintyMinutes:
                        messages.error(
                            request,
                            "Question Asking Limit Exceed, You will be able to ask in 90 minutes",
                        )
                    else:
                        formTags = form.cleaned_data["tags"]
                        gettingBody = form.cleaned_data["body"]
                        ...
                        for typedTags in formTags:
                            ...
                            if request.user.profile.create_tags:
                                ...
                                messages.error(
                                    request,
                                    f"You need atleast 1500 Reputation to create a New Tag - {typedTags}",
                                )
                else:
                    formTags = form.cleaned_data["tags"]
                    gettingBody = form.cleaned_data["body"]
...
def reviewQuestion(request):
    ...
    if (
        taken == False
        and showError == False
        and showNewTagError == False
        and showLessTitleError == False
        and postBody == False
    ):
        allClear = True
    ...
    if request.GET.get("submit") == "mark":
        if post.accepted:
            ...
            if post.date <= timezone.now() - timedelta(minutes=5):
                ...
                que.post_owner == request.user
                and post.date <= timezone.now() - timedelta(days=2)
            ):
                que.is_answer_accepted = True
                que.save()
                # if post.answer_owner != request.user:
...
def question_upvote_downvote(request, question_id):
    ...
    getQ_DownVotes_in_24_Hours = QDownvote.objects.filter(
        downvote_by_q=request.user, date__gt=last_24_hours
    ).count()
    getA_Votes_in_24_Hours = Answer.objects.filter(
        a_vote_ups=request.user, date__gt=last_24_hours
    ).count()
    getA_DownVotes_in_24_Hours = Answer.objects.filter(
        a_vote_downs=request.user, date__gt=last_24_hours
    ).count()
    totalIs = (
        getQ_Votes_in_24_Hours
        + getQ_DownVotes_in_24_Hours
        + getA_Votes_in_24_Hours
        + getA_DownVotes_in_24_Hours
    )
    ...
    if request.GET.get("submit") == "like":
        if QDownvote.objects.filter(
            downvote_by_q=request.user, downvote_question_of=post
        ).exists():

            if (
                downVotedPost.date > upvote_time_limit
                or edited_time > downVotedPost.date
            ):
        ...
        elif QUpvote.objects.filter(
            upvote_by_q=request.user, upvote_question_of=post
        ).exists():
            ...
            if request.user == post.post_owner:
                ...
                if request.user.profile.voteUpPriv:
                    ...
                    post.save()
                    created = QUpvote(upvote_by_q=request.user, upvote_question_of=post)
                    ...
                    getReputationEarnedInLast_24Hours = Reputation.objects.filter(
                        awarded_to=post.post_owner
                    ).aggregate(Sum("answer_rep_C"), Sum("question_rep_C"))
                    # if getReputationEarnedInLast_24Hours
                    d1 = getReputationEarnedInLast_24Hours["question_rep_C__sum"]
                    total_question_rep = (
                        getReputationEarnedInLast_24Hours["question_rep_C__sum"]
                        if d1
                        else 0
                    )
                    s2 = getReputationEarnedInLast_24Hours["answer_rep_C__sum"]
                    total_answer_rep = (
                        getReputationEarnedInLast_24Hours["answer_rep_C__sum"]
                        if s2
                        else 0
                    )
                    ...
                    if (
                        created
                        == QUpvote.objects.filter(upvote_by_q=request.user).first()
                    ):
                        TagBadge.objects.get_or_create(
                            awarded_to_user=request.user,
                            badge_type="BRONZE",
    ...
    # DownVote
        ...
        if QUpvote.objects.filter(
            ...
            if likepost.date > upvote_time_limit or edited_time > likepost.date:
                m = QDownvote(downvote_by_q=request.user, downvote_question_of=post)
                m.save()
                QUpvote.objects.filter(
                    upvote_by_q=request.user, upvote_question_of=post
                ).delete()
                ...
                total_question_rep = (
                    getAlltheReputation["question_rep_C__sum"] if d else 0
                )
        ...
        # return JsonResponse({'action': 'voteError'})
            ...
            if request.user == post.post_owner:
                ...
                if request.user.profile.voteDownPriv:
                    ...
                    if (
                        created
                        == QDownvote.objects.filter(downvote_by_q=request.user).first()
                    ):
                        TagBadge.objects.get_or_create(
                            awarded_to_user=request.user,
                            badge_type="BRONZE",
                    ...
                    s = getAlltheReputation["answer_rep_C__sum"]
                    total_answer_rep = (
                        getAlltheReputation["answer_rep_C__sum"] if s else 0
                    )
                    totalReputation = total_question_rep + total_answer_rep
                    if totalReputation > 2:
...
def answer_upvote_downvote(request, answer_id):
    ...
    getVotedOnQ_Down = Question.objects.filter(
        qdownvote__downvote_by_q=request.user
    ).count()
    ...
    if (
        getVotedOn >= 300
        or getVotedOn_Down >= 300
        or getVotedOnQ >= 300
        or getVotedOnQ_Down >= 300
    ):
        TagBadge.objects.get_or_create(
            awarded_to_user=post.answer_owner,
            badge_type="SILVER",
    ...
    if (
        post.answer_owner == post.questionans.post_owner
        and post.a_vote_ups.count() >= 3
    ):
        TagBadge.objects.get_or_create(
            awarded_to_user=post.answer_owner,
            badge_type="SILVER",
    ...
    subquery = Tag.objects.filter(
        question__answer__answer_owner=post.answer_owner
    ...
    if request.GET.get("submit") == "like":
        if request.user in post.a_vote_downs.all():
            ...
            if request.user.profile.voteUpPriv:
                ...
                if post == Answer.objects.filter(a_vote_ups=request.user).first():
                    TagBadge.objects.get_or_create(
                        awarded_to_user=request.user,
                        badge_type="BRONZE",
                        tag_name="Supporter",
...
def upvote_comment(request, commentq_id):
    ...
    if request.GET.get("submit") == "upVoteCommentIt":
        if request.user in com.com_upvote.all():
            ...
            getCommentsUVotes_MoreThan_5 = (
                CommentQ.objects.filter(commented_by=request.user)
                .annotate(num_comment_upvote=Count("com_upvote"))
                .filter(num_comment_upvote__gte=5)
            )
        ...
        if form.is_valid():
            ...
            if not request.user.profile.edit_questions_answers:
                ...
                data_url = request.build_absolute_uri(
                    reverse(
                        "review:reviewSuggesstedEdit", args=(getReviewingInstance.pk,)
                    )
                )
                Notification.objects.create(
...
def BountiedQuestions(request, query=None):
    ...
    if query == "None":
        questions = Question.objects.filter(
            bounty_date_announced__gte=days_7, is_bountied=True, is_deleted=False
        )
    else:
        questions = Question.objects.filter(
...
def unansweredQuestions(request, query=None):
    if query == "None":
        questions = Question.objects.filter(
            answer__answer_owner=None, is_deleted=False, is_bountied=False
        )
    else:
        questions = (
            Question.objects.filter(
                tags__name__icontains=query, is_deleted=False, is_bountied=False
            )
            .filter(Q(answer__answer_owner=None))
            .distinct()
        )
...
def questions(request):
    questions = (
        ...
        .exclude(is_bountied=True, is_deleted=True)
        .order_by("-date")
    )
        ...
        if "RecentActivity" in this:
            ...
            if query:
                ...
                if bool_1 and bool_2 == False and bool_3 == False:
                    ...
                    questions = (
                        Question.objects.filter(tags__name__icontains=query)
                        .annotate(answers=Count("answer"))
                        .filter(answers=0, is_deleted=False)
                        .order_by("-active_date")
                    )
                ...
                if bool_2 and bool_1 == False and bool_3 == False:
                    ...
                    questions = (
                        Question.objects.filter(tags__name__icontains=query)
                        .filter(answer__accepted=False, is_deleted=False)
                        .distinct()
                        .order_by("-active_date")
                    )
                ...
                if bool_3 and bool_1 == False and bool_2 == False:
                    questions = Question.objects.filter(
                        tags__name__icontains=query, is_bountied=True, is_deleted=False
                    ).order_by("-active_date")
                ...
                if bool_1 == False and bool_2 == False and bool_3 == False:
                    questions = Question.objects.filter(
                        tags__name__icontains=query, is_deleted=False
                    ).order_by("-active_date")
                ...
                if bool_3 and bool_1 == False and bool_2 == False:
                    questions = (
                        Question.objects.all()
                        .exclude(is_bountied=False, is_deleted=True)
                        .order_by("-active_date")
                    )
                    print("Only Third is True")
                ...
                if bool_1 and bool_2 and bool_3 == False:
                    questions = (
                        Question.objects.all()
                        .exclude(answer__accepted=True, is_deleted=True)
                        .annotate(answers=Count("answer"))
                        .filter(answers=0)
                        .order_by("-active_date")
                    )
                ...
                if bool_2 and bool_3 and bool_1 == False:
                    questions = (
                        Question.objects.all()
                        .exclude(
                            is_bountied=False, answer__accepted=True, is_deleted=True
                        )
                        .order_by("-active_date")
                    )
                    print("Second and Third is True")
                ...
                if bool_1 and bool_2 and bool_3:
                    questions = (
                        Question.objects.all()
                        .exclude(is_bountied=False, answer__accepted=True)
                        .annotate(answers=Count("answer"))
                        .filter(answer=0, is_deleted=False)
                        .order_by("-active_date")
                    )
                    print("All Are True")
            ...
            if query:
                ...
                if bool_3 and bool_1 == False and bool_2 == False:
                    questions = Question.objects.filter(
                        tags__name__icontains=query, is_deleted=False, is_bountied=True
                    ).order_by("-date")
                    print("Only Third is True")
                ...
                if bool_1 and bool_2 and bool_3 == False:
                    questions = (
                        ...
                        .annotate(answers=Count("answer"))
                        .filter(answers=0, is_deleted=True)
                        .order_by("-date")
                    )
                    print("First and Second is True")
            ...
            if query:
                ...
                if bool_1 and bool_2 == False and bool_3 == False:
                    ...
                    questions = (
                        ...
                        .annotate(answers=Count("answer"))
                        .filter(answers=0, is_deleted=False)
                        .annotate(mostVotes=Count("qupvote"))
                        .order_by("-mostVotes")
                    )
                ...
                if bool_2 and bool_3 and bool_1 == False:
                    questions = (
                        Question.objects.all()
                        .exclude(
                            is_bountied=False, is_deleted=True, answer__accepted=True
                        )
                        .annotate(mostVotes=Count("qupvote"))
                        .order_by("-mostVotes")
                    )
        ...
        # selected = "MostFrequent"
            ...
            if query:
                ...
                if bool_3 and bool_1 == False and bool_2 == False:
                    questions = Question.objects.filter(
                        tags__name__icontains=query, is_bountied=True, is_deleted=False
                    ).filter(bounty_date_announced__lt=minutes_10)
                    print("Only Third is True")
                ...
                if bool_1 and bool_2 and bool_3 == False:
                    questions = (
                        Question.objects.filter(
                            tags__name__icontains=query, answer__accepted=False
                        )
                        .annotate(answers=Count("answer"))
                        .filter(answers=0)
                        .filter(
                ...
                if bool_1 and bool_3 and bool_2 == False:
                    questions = (
                        Question.objects.filter(
                            tags__name__icontains=query, is_bountied=True
                        )
                        .annotate(answers=Count("answer"))
                        .filter(answers=0)
                        .filter(
    ...
    if request.user.is_authenticated:
        notifics = Notification.objects.filter(noti_receiver=request.user).order_by(
            "-date_created"
        )
    else:
        notifics = ""
