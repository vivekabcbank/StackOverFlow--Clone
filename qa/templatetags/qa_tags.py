...
from qa.models import Answer, BookmarkQuestion, Question, Reputation
...
def calculate_reputation(user_id):
    if user_id.is_authenticated:
        getAlltheReputation = Reputation.objects.filter(awarded_to=user_id).aggregate(
...
def calculateGoldBadges(user_id):
    if user_id.is_authenticated:
        getAllTheGoldBadges = TagBadge.objects.filter(
...
def calculateBronzeBadges(user_id):
    if user_id.is_authenticated:
        getAllTheBronzeBadges = TagBadge.objects.filter(
...
def calculatSilvereBadges(user_id):
    if user_id.is_authenticated:
        getAllTheSilverBadges = TagBadge.objects.filter(
