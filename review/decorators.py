...
from qa.models import Answer, Question, Reputation
    ...
    def awardIt(request, *args, **kwargs):
        if request.user.is_authenticated:
            ...
            getReputationEarnedInLast_24Hours = Reputation.objects.filter(
                awarded_to=getQuestion.post_owner, date_earned__gte=last_24_hours
            ...
            totling2 = (
                getReputationEarnedInLast_24Hours["question_rep_C__sum"] if d1 else 0
            )
            # print(getAlltheReputation)
            s2 = getReputationEarnedInLast_24Hours["answer_rep_C__sum"]
            totlingAnsRep2 = (
                getReputationEarnedInLast_24Hours["answer_rep_C__sum"] if s2 else 0
            )
            ...
            if finalReputation >= 10:
                ...
                print(
                    "Award the Badge that have to award at 200 reputation earn in 24 Hours"
                )
            print("\n")
    ...
    def redirectFunc(request, *args, **kwargs):
        ...
        if request.user.is_authenticated:
            if request.user.profile.review_close_votes:
                ...
                getReviewing_item.question_to_view
                and request.user == getQuestion.post_owner
            ):
                print("Redirect through Second Statement")
                return function(request, *args, **kwargs)
            elif (
                getReviewing_item.answer_to_view_if
                and request.user == getAnswer.answer_owner
            ):
