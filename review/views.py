...
from qa.models import CommentQ, Reputation
from tagbadge.models import TagBadge

from .decorators import (
    required_500_RepToReview,
    required_2000_RepToReview,
    required_3000_RepToReview,
)
from .forms import (
    AnswerReviewForm,
    CloseForm_Q,
    FlagPostForm,
    LateAnswerReviewForm,
    LowQualityReviewForm,
    QuestionReviewForm,
    ReviewCloseForm,
    ReviewFlagCommentForm,
    ReviewReOpenForm,
    SuggesstedEditForm,
    VoteToReOpenForm,
)
...
def reOpen_Question_History(request, reviewquestionreopenvotes_id):
    getReviewItem = get_object_or_404(
        ReviewQuestionReOpenVotes, pk=reviewquestionreopenvotes_id
    )
...
def suggesstedEditHistory_Question(request, reviewquestionedit_id):
    getReviewItem = get_object_or_404(ReviewQuestionEdit, pk=reviewquestionedit_id)
...
def reviewFlagComments(request, reviewflagcomment_id):
    commentToReview = get_object_or_404(ReviewFlagComment, id=reviewflagcomment_id)
        ...
        if form.is_valid():
            ...
            if formData == "DELETE_IT":
                create_a_NewInstance = FlagComment.objects.create(
                    comment_flagged_by=request.user,
                    comment_of=data,
                    why_flagging="DeleteIt",
                )
                ...
                getFlaggerUserProfile.comment_flagged_by.profile.helpful_flags_counter += (
                    1
                )
            ...
            if counting_2 >= 1:
                return redirect(
                    "review:reviewFlagComments", reviewflagcomment_id=next_flag.id
                )
            else:
    ...
    getThisItemFromReview = ReviewFlagComment.objects.filter(flag_of=data).first()
...
def reviewLowQualityPosts(request, reviewlowqualityposts_id):
    getReviewItem = get_object_or_404(
        ReviewLowQualityPosts, pk=reviewlowqualityposts_id
    )
    ...
    if request.method == "POST":
        form = LowQualityReviewForm(
            instance=getReviewItem, data=request.POST, files=request.FILES
        )
        if form.is_valid():
            formData = form.cleaned_data["reviewActions"]
            if getReviewItem.is_question:
                getQuestion = Question.objects.get(reviewlowqualityposts=getReviewItem)
                ...
                if counting_2 >= 1:
                    ...
                    messages.error(request, "No More Posts to Review Low Quality Posts")
                    return redirect("profile:home")

            else:
                getAnswer = Answer.objects.get(reviewlowqualityposts=getReviewItem)
                getLowQualityItem = LowQualityPostsCheck.objects.filter(
    ...
    if getReviewItem.is_question:
        ...
        getReviewersByVote = LowQualityPostsCheck.objects.filter(low_is=data).first()
        lastVoteWas = ReviewLowQualityPosts.objects.filter(
            is_question=data, is_reviewed=True
        ).last()
        ...
        likeDownpost = data.qdownvote_set.filter(downvote_by_q=request.user).first()
        ...
        lastVoteWas = ReviewLowQualityPosts.objects.filter(
            is_answer=data, is_reviewed=True
        ).last()
        ...
        likepost = data.questionans.qupvote_set.filter(upvote_by_q=request.user).first()
...
def reviewClosedQuestions(request, reviewclosevotes_id):
    ...
    getCloseHistorys = (
        CloseQuestionVotes.objects.filter(question_to_closing=data)
        .exclude(ended=True)
        .first()
    )
    ...
    if request.method == "POST":
        if counting >= 1:
            form = ReviewCloseForm(
                instance=questionClose, data=request.POST, files=request.FILES
            )
            if form.is_valid():
                new_post = form.save(commit=False)
                formData = form.cleaned_data["reviewActions"]
                ...
                if formData == "Close":
                    ...
                    if getLast_completed_reason_of_review:
                        if (
                            getLast_completed_reason_of_review.how_Ended
                            == "THROUGH_LEAVE_OPEN"
                        ):
                            print("First Statement is Excecuting")
                            if (
                                getCloseHistorys.how_many_votes_on_Close
                                >= getCloseHistorys.how_many_votes_on_Leave_open + 3
                            ):
                                ...
                                if (
                                    getCloseHistorys.flagged_by.profile.helpful_flags_counter
                                    >= 80
                                ):
                                ...
                                data.save()
                                questionClose.finalResult = (
                                    questionClose.review_of.why_closing
                                )
                            ...
                            if (
                                getCloseHistorys.how_many_votes_on_Close >= 3
                                and getCloseHistorys.how_many_votes_on_Close
                        ...
                        if (
                            getCloseHistorys.how_many_votes_on_Close >= 3
                            and getCloseHistorys.how_many_votes_on_Close
                            > getCloseHistorys.how_many_votes_on_Leave_open
                        ):
                            questionClose.is_completed = True
                            questionClose.reviewed_by.add(request.user)
                            print(
                            ...
                            getCloseHistorys.user.profile.save()
                            if (
                                getCloseHistorys.user.profile.helpful_flags_counter
                                >= 80
                            ):
                                # createMainTag = Tag.objects.get_or_create(name="Deputy")
                                TagBadge.objects.get_or_create(
                ...
                # COMMENTED OUT NOT-WORKING - START
                    ...
                    if getLast_completed_reason_of_review:
                        if (
                            ...
                            if (
                                getCloseHistorys.how_many_votes_on_Leave_open >= 3
                                and getCloseHistorys.how_many_votes_on_Leave_open
                                > getCloseHistorys.how_many_votes_on_Close
                            ):
            ...
            messages.error(request, "No More Posts to Review Close Question Votes")
            return redirect("profile:home")

    else:
        form = ReviewCloseForm(
            request.POST or None, request.FILES or None, instance=questionClose
        )

    getAll_Votes_required_in_close_and_completed = (
        CloseQuestionVotes.objects.filter(question_to_closing=data)
        .filter(
            Q(why_closing="NEED_TO_MORE_FOCUSED")
            | Q(why_closing="DUPLICATE")
            | Q(why_closing="NEED_ADDITIONAL_DETAILS")
            | Q(why_closing="OPINION_BASED")
            | Q(why_closing="Close")
            | Q(why_closing="Leave_open")
        )
        .exclude(ended=True)
    )

    if request.method == "POST":
        editQ = UpdateQuestion(instance=data, data=request.POST, files=request.FILES)
    ...
    getReviewersByVote = CloseQuestionVotes.objects.filter(question_to_closing=data)
...
def reviewFlagPosts(request, reviewflagpost_id):
        ...
        if form.is_valid():
            ...
            if getReviewItem.flag_question_to_view:
                ...
                getFlagVoteItem = FlagPost.objects.filter(
                    question_forFlag=getQuestion, ended=False
                ).first()
                ...
                if counting >= 1:
                    return redirect(
                        "review:reviewFlagPosts", reviewflagpost_id=next_flag_post.id
                    )
                else:
                ...
                getFlagVoteItem = FlagPost.objects.filter(
                    answer_forFlag=getAnswer, ended=False
                ).first()
    ...
    if getReviewItem.flag_question_to_view:
        ...
        getReviewersByVote = FlagPost.objects.filter(question_forFlag=data).first()
        lastVoteWas = ReviewFlagPost.objects.filter(
            flag_question_to_view=data, flag_is_reviewed=True
        ).last()
        upvotesCount = data.qupvote_set.all().count()
        downvotesCount = data.qdownvote_set.all().count()
        allToShowVotes = upvotesCount - downvotesCount
        ...
        data = Answer.objects.filter(reviewflagpost=getReviewItem).first()
        getThisItemFromReview = ReviewFlagPost.objects.filter(
            flag_answer_to_view_if=data
        ).first()
        getReviewersByVote = FlagPost.objects.filter(answer_forFlag=data).first()
        lastVoteWas = ReviewFlagPost.objects.filter(
            flag_answer_to_view_if=data, flag_is_reviewed=True
        ).last()
        getQuestionFromAnswer = Question.objects.get(answer=data)
        # Try to implement this in models
        likepost = data.questionans.qupvote_set.filter(upvote_by_q=request.user).first()
...
def rewardPrivielege(request, which_user):
    ...
    """
    getAlltheReputation = Reputation.objects.filter(awarded_to=which_user).aggregate(
    ...
    if totalReputation >= 10:
        ...
        PrivRepNotification.objects.get_or_create(
            for_user=which_user, privilegeURL="#", type_of_PrivNotify="Privilege_Earned"
        )
        PrivRepNotification.objects.get_or_create(
            for_user=which_user,
            privilegeURL="#",
...
def reviewSuggesstedEdit(request, reviewquestionedit_id):
    ...
    if getReviewItem.question_to_view:
        ...
        if request.method == "POST":
            edit_Q_Form = UpdateQuestion(
                instance=getQuestion, data=request.POST, files=request.FILES
            )
            if edit_Q_Form.is_valid():
                ...
                return redirect("review:review_FirstQns", question_id=next_question.id)

        else:
            edit_Q_Form = UpdateQuestion(
                request.POST or None, request.FILES or None, instance=getQuestion
            )
    else:
        edit_Q_Form = ""
    ...
    if getReviewItem.answer_to_view_if:
        ...
        if request.method == "POST":
            editAnswerForm = EditAnswerForm(
                instance=getAnswer, data=request.POST, files=request.FILES
            )
            ...
            editAnswerForm = EditAnswerForm(
                request.POST or None, request.FILES or None, instance=getAnswer
            )
    else:
        editAnswerForm = ""
    ...
    if request.method != "POST":
        ...
        if getReviewItem.question_to_view:
            ...
            getVoteItem = QuestionEditVotes.objects.filter(
                edited_question=getQuestion, is_completed=False, rev_Action=None
            ).first()
            ...
            if getQuestion.post_owner == request.user:
                ...
                if formData == "Approve":
                    ...
                    getReviewItem.save()
                    if (
                        getVoteItem.how_many_votes_on_approve
                        > getVoteItem.how_many_votes_on_reject
                        and getVoteItem.how_many_votes_on_approve > 2
                    ):
                    ...
                    if counting_2 >= 1:
                        ...
                        messages.error(request, "No More Suggessted Edits to Review")
                        return redirect("profile:home")
                    ...
                    if (
                        getVoteItem.how_many_votes_on_reject
                        > getVoteItem.how_many_votes_on_approve
                        and getVoteItem.how_many_votes_on_reject > 2
                    ):
                        # getDat = getQuestion.history.first()
                        ...
                        if edit_Q_Form.is_valid():
                            ...
                            next_question = (
                                Question.objects.filter(
                                    firstquestionreview__QuestionReviewBy__isnull=True
                                )
                                .order_by("id")
                                .first()
                            )
                            return redirect(
                                "review:review_FirstQns", question_id=next_question.id
                            )
        ...
        # ANSWER REVIEW
            ...
            getAnswer = Answer.objects.get(reviewquestionedit=getReviewItem)
            getVoteItem = QuestionEditVotes.objects.filter(
                edited_answer=getAnswer, rev_Action=None, is_completed=False
            ).first()
            if getAnswer.answer_owner == request.user:
                if formData == "Approve":
                    ...
                    return redirect(
                        "qa:questionDetailView", pk=getAnswer.questionans.id
                    )
    ...
    if getReviewItem.question_to_view:
        ...
        getThisItemFromReview = ReviewQuestionEdit.objects.filter(
            question_to_view=data, is_reviewed=False
        ).first()
        # Bug in This
        getReviewersByVote = QuestionEditVotes.objects.filter(
            edited_question=data, is_completed=False
        ).exclude(rev_Action=None)
        lastVoteWas = ReviewQuestionEdit.objects.filter(
            question_to_view=data, is_reviewed=True
        ).last()
        ...
        for anss in answers_of_questions:
            ...
            if getLastEditVotes:
                if (
                    getLastEditVotes.rev_Action == "Approve"
                    and getLastEditVotes.is_completed
                    or getLastEditVotes.rev_Action == "Approve_Through_Edit"
                    and getLastEditVotes.is_completed
                ):
                    getTheOriginal = anss
        ...
        getThisItemFromReview = ReviewQuestionEdit.objects.filter(
            answer_to_view_if=data, is_reviewed=False
        ).first()
        getReviewersByVote = QuestionEditVotes.objects.filter(
            edited_answer=data, is_completed=True
        ).first()
        lastVoteWas = ReviewQuestionEdit.objects.filter(answer_to_view_if=data).last()
        getQuestionFromAnswer = Question.objects.get(answer=data)
...
def retract_Close_Flag(request, question_id):
    ...
    getCreatedObject_if_Exist = (
        CloseQuestionVotes.objects.filter(question_to_closing=getQuestion)
        .exclude(ended=True)
        .exists()
    )
    ...
    getReviewingObject = ReviewCloseVotes.objects.filter(
        review_of=getCreatedObject_2, is_completed=False
    ).first()

    if getCreatedObject_if_Exist:
        if (
            getCreatedObject_2.user == request.user
            and getCreatedObject_2.how_many_votes_on_Close < 1
        ):
...
def retract_Flag_Form(request, question_id):
    ...
    getCreateObject_If_Exist = (
        FlagPost.objects.filter(question_forFlag=getQuestion)
        .exclude(ended=True)
        .exists()
    )

    getCreatedObject_2 = (
        FlagPost.objects.filter(flagged_by=request.user, question_forFlag=getQuestion)
        .exclude(ended=True)
        .first()
    )

    getReviewingObject = ReviewFlagPost.objects.filter(
        flag_question_to_view=getQuestion, flag_is_reviewed=True
    ).first()

    if getCreatedObject_if_Exist:
        if (
            getCreatedObject_2.flagged_by == request.user
            and getCreatedObject_2.how_many_votes_on_spamANDRude < 1
            or getCreatedObject_2.how_many_votes_on_notAnAnswer < 1
            or getCreatedObject_2.how_many_votes_on_others < 1
        ):
            getCreatedObject_2.delete()
            getReviewingObject.delete()
        else:
...
def review_FirstAns(request, answer_id):
    ...
    if request.method == "POST":
        editForm = EditAnswerForm(instance=data, data=request.POST, files=request.FILES)
    ...
    getThisItemFromReview = FirstAnswerReview.objects.filter(answerReview=data).first()
...
def review_LateAnswers(request, answer_id):
    ...
    counting = (
        Answer.objects.filter(
            questionans__date__gt=timezone.now() - timedelta(hours=100)
        )
        .filter(date__gt=isOlderThanFiveHours)
        .count()
    )
    ...
    if request.method == "POST":
        if counting >= 1:
            ...
            if form.is_valid():
                ...
                next_blog = (
                    Answer.objects.filter(
                        lateanswerreview__L_AnswerActions__isnull=True
                    )
                ...
                getAllLowQualityTask = LateAnswerReview.objects.filter(
                    L_AnswerReviewdBy=request.user
                ).count()
                ...
                if counting >= 1:
                    return redirect("review:review_LateAnswers", answer_id=next_blog.id)
                else:
    ...
    getThisItemFromReview = LateAnswerReview.objects.filter(L_answerReview=data).first()
...
def edit_in_Closing_Ajax(request, question_id):
    data = get_object_or_404(Question, pk=question_id)
    if is_ajax(request) and request.method == "POST":
        editForm = UpdateQuestion(instance=data, data=request.POST, files=request.FILES)
...
def editFromReview(request, answer_id):
    ...
    if request.method == "POST":
        form = EditAnswerForm(instance=post, data=request.POST, files=request.FILES)
        if form.is_valid():
            form.save()
            next_blog = (
                Answer.objects.filter(firstanswerreview__actions__isnull=True)
...
def review_answer_page(request):
    ...
    counting = Answer.objects.filter(firstanswerreview__actions__isnull=True).count()
...
def reOpen_Question_Review(request, reviewquestionreopenvotes_id):
    questionUNClose = get_object_or_404(
        ReviewQuestionReOpenVotes, id=reviewquestionreopenvotes_id
    )
    data = Question.objects.get(reviewquestionreopenvotes=questionUNClose)
    getUNCloseHistorys = (
        ReOpenQuestionVotes.objects.filter(question_to_opening=data)
        .filter(
            Q(why_opening="IT_IS_BETTER_NOW")
            | Q(why_opening="CLOSED_BY_MISTAKE")
            | Q(why_opening="OTHER")
            | Q(why_opening="Open")
        )
        .exclude(ended=True)
    ...
    if request.method != "POST":
        form = ReviewReOpenForm(
            instance=questionUNClose, data=request.POST, files=request.FILES
        )
        ...
        if form.is_valid():
            ...
            if formData == "OPEN":
                ...
                if getLast_completed_reason_of_review:
                    ...
                    if (
                        getLast_completed_reason_of_review.what_happend
                        == "Ended_Through_Leave_Closed"
                    ):
                        if (
                            getUNCloseHistorys.how_many_votes_on_Open
                            > getUNCloseHistorys.how_many_votes_on_Leave_close + 2
                        ):
                        ...
                        getUNCloseHistorys.how_many_votes_on_Open >= 3
                        and getUNCloseHistorys.how_many_votes_on_Open
                        > getUNCloseHistorys.how_many_votes_on_Leave_close
                    ):
                        questionUNClose.is_completed = True
                        questionUNClose.what_happend = "Ended_Through_Open"
                        # REOPEN THE QUESTION - HERE -
                ...
                if counting_2 >= 1:
                    ...
                    messages.error(request, "No More ReOpen Post Votes to Review")
                    return redirect("profile:home")
                ...
                if getLast_completed_reason_of_review:
                    if (
                        ...
                        if (
                            getUNCloseHistorys.how_many_votes_on_Leave_close
                            > getUNCloseHistorys.how_many_votes_on_Open + 2
                        ):
                        ...
                        getUNCloseHistorys.how_many_votes_on_Leave_close >= 3
                        and getUNCloseHistorys.how_many_votes_on_Leave_close
                        > getUNCloseHistorys.how_many_votes_on_Open
                    ):
                        questionUNClose.is_completed = True
                        questionUNClose.what_happend = "Ended_Through_Leave_Closed"
    ...
    getReviewersByVote = ReOpenQuestionVotes.objects.filter(question_to_opening=data)
