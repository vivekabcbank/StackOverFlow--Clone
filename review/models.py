...
from qa.models import Answer, CommentQ, Question
...
class FirstAnswerReview(models.Model):
    ...
    actions = models.CharField(
        max_length=30, choices=REVIEW_ANSWER_ACTION_CHOICES, null=True
    )
...
class FirstQuestionReview(models.Model):
    ...
    date = models.DateTimeField(auto_now_add=True)
    questionActions = models.CharField(
        max_length=30, choices=REVIEW_QUESTION_ACTIONS_CHOICES, null=True
    )
...
class LateAnswerReview(models.Model):
    ...
    L_AnswerActions = models.CharField(
        max_length=30,
        choices=LATE_ANSWER_ACTIONS_CHOICES,
        null=True,
...
class QuestionEditVotes(models.Model):
    ...
    edited_question = models.ForeignKey(
        Question, on_delete=models.CASCADE, null=True, blank=True
    )
    edited_answer = models.ForeignKey(
...
class ReviewQuestionEdit(models.Model):
    question_to_view = models.ForeignKey(
        Question, on_delete=models.CASCADE, blank=True, null=True
    )
    answer_to_view_if = models.ForeignKey(
    ...
    queue_of = models.ForeignKey(
        QuestionEditVotes, on_delete=models.CASCADE, blank=True, null=True
    )
    date_reviewed = models.DateTimeField(auto_now_add=True)
    reviewActions = models.CharField(
        max_length=30, choices=EDIT_REVIEW_ACTIONS, blank=True, null=True
    )
...
class CloseQuestionVotes(models.Model):
    ...
    def __str__(self):
        return (
            f"{self.user} - {self.question_to_closing.title} [WHY] - {self.why_closing}"
        )
...
class ReviewCloseVotes(models.Model):
    question_to_closed = models.ForeignKey(
        Question, on_delete=models.CASCADE, default=""
    )
    review_of = models.ForeignKey(
        CloseQuestionVotes, on_delete=models.CASCADE, default="", null=True
    )
    reviewActions = models.CharField(
        max_length=30, choices=CLOSE_REVIEW_ACTION_CHOICES, blank=True, null=True
    )
    reviewed_by = models.ManyToManyField(User, related_name="reviewed_by")
    is_completed = models.BooleanField(default=False)
    how_Ended = models.CharField(
        max_length=30, choices=ENDED_CHOICES, blank=True, null=True
    )
...
class ReviewQuestionReOpenVotes(models.Model):
    ...
    review_of = models.ForeignKey(
        ReOpenQuestionVotes, on_delete=models.CASCADE, default="", null=True
    )
    reviewActions = models.CharField(
        max_length=30, choices=REVIEW_REOPEN_ACTION_CHOICES, blank=True, null=True
    )
...
SUGGEST_CHOICES = [("User", "User"), ("Automatic", "Automatic")]
...
class LowQualityPostsCheck(models.Model):
    ...
    low_is = models.ForeignKey(
        Question, on_delete=models.CASCADE, null=True, blank=True
    )
    low_ans_is = models.ForeignKey(
        Answer, on_delete=models.CASCADE, null=True, blank=True
    )
...
class ReviewLowQualityPosts(models.Model):
    ...
    reviewActions = models.CharField(
        max_length=30, choices=LOW_QUALITY_CHOICES, blank=True, null=True
    )
    is_answer = models.ForeignKey(
        Answer, on_delete=models.CASCADE, blank=True, null=True
    )
    is_question = models.ForeignKey(
        Question, on_delete=models.CASCADE, blank=True, null=True
    )
...
Q_Flags_Choices = [
    ...
    ),  # Can only seen by moderators = POST
    ...
    (
        "VERY_LOW_QUALITY",
        "Very Low Quality",
    ),  # Will be pushed to Low Quality Posts = QUESTION
    ...
    (
        "NEED_MORE_FOCUS",
        "Need More Focus# = QUESTION",
    ),  # = QUESTION # ----Close Review----
    (
    ...
    (
        "BLANTANLTY_OR_CLARITY",
        "Blantanlity or Clarity",
    ),  # = QUESTION # ----Close Review----
]
...
class FlagPost(models.Model):
    ...
    question_forFlag = models.ForeignKey(
        Question, on_delete=models.CASCADE, blank=True, null=True
    )
    answer_forFlag = models.ForeignKey(
        Answer, on_delete=models.CASCADE, blank=True, null=True
    )
...
class ReviewFlagPost(models.Model):
    flag_question_to_view = models.ForeignKey(
        Question, on_delete=models.CASCADE, blank=True, null=True
    )
    flag_answer_to_view_if = models.ForeignKey(
        Answer, on_delete=models.CASCADE, blank=True, null=True
    )
    flag_reviewed_by = models.ForeignKey(
        User, on_delete=models.CASCADE, blank=True, null=True
    )
    flag_of = models.ForeignKey(
        FlagPost, on_delete=models.CASCADE, blank=True, null=True
    )
...
class ReviewFlagComment(models.Model):
    ...
    review_of = models.ForeignKey(
        FlagComment, on_delete=models.CASCADE, default="", null=True
    )
